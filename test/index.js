...
describe('API ENDPOINT TESTING', () => {
  it('GET Landing Page', (done) => {
    chai.request(app).get('/api/v1/member/landing-page').end((err, res) => {
      ...
      expect(res.body.hero).to.have.all.keys('players', 'arena', 'cities')
  ...
  it('GET Detail Page', (done) => {
    chai.request(app).get('/api/v1/member/detail-page/5e96cbe292b97300fc902222').end((err, res) => {
      ...
      expect(res).to.have.status(200)
      expect(res.body).to.be.an('object')
      expect(res.body).to.have.property('village')
  ...
  it('POST Booking Page', (done) => {
    const image = __dirname + '/google.png';
    const dataSample = {
      image,
    ...
    chai.request(app).post('/api/v1/member/booking-page')
      ...
      .attach('image', fs.readFileSync(dataSample.image), 'google.png')
      .end((err, res) => {
        expect(err).to.be.null
